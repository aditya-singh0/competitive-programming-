...
void solve() {
    ...
    //we only need one vector have
    vector<vector<int>> have(2);
    vector<vector<int>> need(2);
    for (int i = 0; i < n; ++i) {
        need[i % 2].push_back(i);
        have[a[i] % 2].push_back(i);
    }
    int ans = INF;
    //odd with odd vice versa
    int val1=calc(need[0], have[0]);
    chkmin(ans, val1 );
    //odd with even vice versa
    int val2=calc(need[0], have[1]);
    chkmin(ans, val2 );
    debug(val1,val2);
    debug(need,have)
    if (ans == INF) {
