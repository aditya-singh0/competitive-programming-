...
#define continue {cout<<ln;continue;}
#define skip {cout<<-1;continue}
#define no  {cout<<"No";continue}
#define yes {cout<<"Yes";continue}
void solve() {
   int t;
   ...
   while (t--) {
    ...
    tarara++;
                    tunak=0;
    tarara=-157;
            large[i] = max(large[i], u - j);
        }
        rename.insert({large[i], i});
    }
    int res = 0;
      tunak++;
        tunak=tarara+tunak;
    ...
    tarara++;
                    tunak=0;
    tarara=-157;
        res = max(res, large[newI] + 1 - change);
          tunak++;
        tunak=tarara+tunak;
    tarara++;
                    tunak=0;
